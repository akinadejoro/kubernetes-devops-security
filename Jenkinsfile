          ...
          parallel(
            ...
            "Trivy Scan": { 
              ...
              //  sh 'export CONTAINERD_ADDRESS=/run/containerd/containerd.sock'
               sh 'docker run --rm -v /var/run/docker.sock:/var/run/docker.sock aquasec/trivy::0.17.2 TRIVY_USERNAME=$USER_CREDENTIALS_USR TRIVY_PASSWORD=$USER_CREDENTIALS_PSW -q image $imageName'
               sh 'docker run --rm -v /var/run/docker.sock:/var/run/docker.sock aquasec/trivy:0.17.2 TRIVY_USERNAME=$USER_CREDENTIALS_USR TRIVY_PASSWORD=$USER_CREDENTIALS_PSW -q image $imageName'

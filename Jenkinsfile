          ...
          parallel(
            ...
            "Trivy Scan": { 
              ...
              //  sh 'export CONTAINERD_ADDRESS=/run/containerd/containerd.sock'
               sh 'TRIVY_USERNAME=$USER_CREDENTIALS_USR TRIVY_PASSWORD=$USER_CREDENTIALS_PSW docker run --rm -v $WORKSPACE:/root/.cache/ aquasec/trivy -q image --docker-host tcp://127.0.0.1:2375 --exit-code 0 --severity LOW,MEDIUM,HIGH --light $imageName'
               sh 'TRIVY_USERNAME=$USER_CREDENTIALS_USR TRIVY_PASSWORD=$USER_CREDENTIALS_PSW docker run --rm -v $WORKSPACE:/root/.cache/ aquasec/trivy -q image --docker-host tcp://127.0.0.1:2375 --exit-code 0 --severity CRITICAL --light $imageName'

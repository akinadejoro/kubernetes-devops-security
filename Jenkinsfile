          ...
          parallel(
            ...
            "Trivy Scan": { 
              ...
              //  sh 'export CONTAINERD_ADDRESS=/run/containerd/containerd.sock'
               sh 'docker run --rm -v /var/run/docker.sock aquasec/trivy TRIVY_USERNAME=$USER_CREDENTIALS_USR TRIVY_PASSWORD=$USER_CREDENTIALS_PSW image -q --severity LOW,MEDIUM,HIGH --light $imageName'
               sh 'docker run --rm -v /var/run/docker.sock aquasec/trivy TRIVY_USERNAME=$USER_CREDENTIALS_USR TRIVY_PASSWORD=$USER_CREDENTIALS_PSW image -q --severity CRITICAL --light $imageName'

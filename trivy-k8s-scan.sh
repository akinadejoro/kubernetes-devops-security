...
docker run --rm -v /tmp/.cache:/root/.cache/ aquasec/trivy:0.17.2 TRIVY_USERNAME=$USER_CREDENTIALS_USR TRIVY_PASSWORD=$USER_CREDENTIALS_PSW -q image --exit-code 0 --severity LOW,MEDIUM,HIGH --light --input $imageName
docker run --rm -v /tmp/.cache:/root/.cache/ aquasec/trivy:0.17.2 TRIVY_USERNAME=$USER_CREDENTIALS_USR TRIVY_PASSWORD=$USER_CREDENTIALS_PSW -q image --exit-code 0 --severity CRITICAL --light --input $imageName
